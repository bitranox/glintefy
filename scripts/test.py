...
def _check_dependencies_and_prompt_update() -> None:
    ...
    This function runs at the end of the test suite to inform users about
    outdated dependencies and offer to update them automatically.
    """
...
def _check_installed_dependencies() -> None:
    ...
    This is a separate step that verifies the current interpreter has the
    required dependencies installed at the correct versions.
    """
...
def run_tests(*, coverage: str = "on", verbose: bool = False, strict_format: bool | None = None) -> None:
    ...
    # Step 0: Ensure pip is up-to-date (first thing we do)
    _check_and_update_pip()

    sync_metadata_module(PROJECT)
    bootstrap_dev()

    # Step 1: Check pyproject.toml dependencies against PyPI (with update prompt)
    _check_dependencies_and_prompt_update()

    # Step 2: Check installed packages meet requirements (with install prompt)
