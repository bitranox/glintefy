...
logger = get_mcp_logger("btx_fix_mcp.llm_client")
...
class InternalLLMClient:
    ...
    def _get_env_api_key(self) -> str | None:
        ...
        return os.getenv("BTX_FIX_MCP_ANTHROPIC_API_KEY") or os.getenv("ANTHROPIC_API_KEY")
    ...
    def client(self):
        ...
        if self._client is None:
            if not self.api_key:
                raise ValueError("Anthropic API key not configured. Set BTX_FIX_MCP_ANTHROPIC_API_KEY or configure in config.toml")
...
Respond with ONLY the JSON object, no other text."""
    ...
    def generate_commit_message(
        ...
        Example:
            ...
            ...     files_changed=["src/btx_fix_mcp/foo.py"]
            ... )
        """
